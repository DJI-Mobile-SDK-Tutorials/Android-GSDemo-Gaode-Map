...
Next, click the "添加新Key" button on the upper right corner of "GSDemo" Application. Enter the info as you want, for the "发布版安全码：SHA1" and "调试版安全码SHA1" fields, please enter the SHA-1 key we just generate in the above steps. 
 ...
 ![createAMAPKey](./Images/createAMAPKey.png)
...
For more details of description on the permissions, please refer to <http://lbs.amap.com/api/android-sdk/guide/project/>.
...
![openModuleSettings](./Images/openModuleSettings.png)
...
![selectLibsPackage](./Images/selectLibsPackage.png)

Then press "Finish" button to import the AMap_2DMap JAR package:

![createNewModule](./Images/createNewModule.png)
...
**2.** We override the `onCreate()` method to invoke `initUI()` method to initialize the UI. Then invoke the `initMapView()` method to create the MapView and add a marker of Shenzhen, China here. So when the Gaode map is loaded, you will see a blue pin tag on Shenzhen, China.
